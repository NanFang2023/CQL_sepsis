...
class ConservativeSAC(object):
    ...
    def get_default_config(updates=None):
        ...
        config.policy_lr = 1e-5   #3e-4
        config.qf_lr = 1e-5       #3e-4
        config.optimizer_type = 'adam'
        config.soft_target_update_rate = 1e-2  #5e-3
        ...
        config.cql_temp = 1.0
        config.cql_min_q_weight = 1.0  # 5.0
    ...
    def __init__(self, config, policy, qf1, qf2, target_qf1, target_qf2):
        ...
        self.qf1_optimizer = optimizer_class(
            self.qf1.parameters(), lr = self.config.qf_lr
        )

        self.qf2_optimizer = optimizer_class(
            self.qf2.parameters(), lr = self.config.qf_lr
        )
    ...
    def train(self, batch, bc=False):
        ...
        new_actions, log_pi = self.policy(observations) # 得到概率分布log_pi

        if self.config.use_automatic_entropy_tuning: # True 自动调整alpha
        ...
        if self.config.backup_entropy:  # false
        ...
        if not self.config.use_cql:
            ...
            cql_random_actions = actions.new_empty((batch_size, self.config.cql_n_actions, action_dim), requires_grad=False).uniform_(0, 1)
            ...
            cql_q1_rand = self.qf1(observations, cql_random_actions)  # 当前状态下，随机行动的Q值
            cql_q2_rand = self.qf2(observations, cql_random_actions)
            cql_q1_current_actions = self.qf1(observations, cql_current_actions) # 当前状态下，当前行动对应的Q值
            ...
            if self.config.cql_importance_sample: # TRUE 重要性采样
            ...
            cql_qf1_ood = torch.logsumexp(cql_cat_q1 / self.config.cql_temp, dim=1).mean() * self.config.cql_min_q_weight * self.config.cql_temp
            cql_qf2_ood = torch.logsumexp(cql_cat_q2 / self.config.cql_temp, dim=1).mean() * self.config.cql_min_q_weight * self.config.cql_temp

            min_qf1_loss = cql_qf1_ood - q1_pred.mean() * self.config.cql_min_q_weight
            min_qf2_loss = cql_qf2_ood - q2_pred.mean() * self.config.cql_min_q_weight
            ...
            """Subtract the log likelihood of data　减去数据的对数似然"""
            ...
            # if self.config.cql_lagrange: # false
